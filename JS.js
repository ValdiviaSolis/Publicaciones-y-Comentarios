...
menu.addEventListener('change', () => {
    fetch(`https://jsonplaceholder.typicode.com/users/${menu.value}`)
        ...
        .then(json => {
            ...
            info.innerHTML += `<p>ID DEL USUARIO: ${json.id}<br>NOMBRE DEL USUARIO: ${json.name}<br>DIRECCION: ${json.address.street}<br>TELEFONO: ${json.phone}<br>E-MAIL: ${json.email}<br>SITIO WEB: ${json.website}<br><br></p>`
            console.log(json)
        })
    ...
    fetch(`https://jsonplaceholder.typicode.com/posts?userId=${menu.value}`)
        ...
        .then(json => {
            ...
            json.forEach(pub => {
                ...
                <button class='botoncom' onclick='comentarios(${pub.id})'>VER COMENTARIO</button><button class='botoncom' onclick='ocultar(${pub.id})'>OCULTAR COMENTARIO </button><br>
                </div>
